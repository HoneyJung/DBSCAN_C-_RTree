...
public:
  ...
  vector<Point*> Search_neighbors(ELEMTYPE* XandY, ELEMTYPE eps2,int dim);
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],vector<Point*>& result);
  ...
  public:
    ...
    Branch& operator*()
    {
      ...
      return curTos.m_node->m_branch[curTos.m_branchIndex];
    }
    ...
    {
      StackElement& curTos = m_stack[m_tos - 1];
      cout << curTos.m_node->m_level << endl;
    }
  private:
...
public: //originally it was private.
  Node* AllocNode();
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, vector<Point*>& result);
...
RTREE_TEMPLATE
vector<Point*> RTREE_QUAL::Search_neighbors(ELEMTYPE* XandY, ELEMTYPE eps, int dim)
{
  double search_min[2] = {XandY[0],XandY[1]};
  double search_max[2] = {XandY[0],XandY[1]};
  vector<Point*> result;
  for(int i=0; i<dim; i++)
  {
    search_min[i] -= eps;
  ...
  Search(search_min, search_max,result);
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], vector<Point*>& result){
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
    ASSERT(a_min[index] <= a_max[index]);
...
#endif //_DEBUG
  //cout << a_min[0] << endl << a_min[1] << endl << a_max[0] << endl << a_max[1] << endl;
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, result);
  //cout << "size " << result.size() << endl;
  //cout << "foundcount " << foundCount << endl;

  //cout << result[0].m_rect.m_min[0] << endl;
  return foundCount;
}
...
int RTREE_QUAL::Count()
{
  ...
  CountRec(m_root, count);
    ...
    // Temp2.m_max[0] = 4;
    // Temp2.m_max[1] = 4;
    // cout << "overlaped? " <<  Overlap(&Temp1,&Temp2) << endl;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, vector<Point*>& result)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  //vector<Branch> neighbors;
  //result.push_back(a_node->m_branch[0]);
  //cout << "rect" << a_rect->m_min[0] << ' ' << a_rect->m_min[1] << ' ' << a_rect->m_max[0] << ' ' << a_rect->m_max[1]  << endl;
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, result);
      }
    }
  }
  else
  {
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        //result.push_back((a_node->m_branch[index].m_data));      
        result.push_back((&(a_node->m_branch[index].point)));
      }
    }
  }
